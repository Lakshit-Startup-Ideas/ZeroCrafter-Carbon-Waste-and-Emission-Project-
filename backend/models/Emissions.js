...
emissionsSchema.pre('save', function (next) {
  ...
  // Calculate emissions
  const calculated = emissionsCalculator.calculateTotal(
    energyData,
    this.wasteData
  );
...
emissionsSchema.virtual('totalEnergyConsumption').get(function () {
  const electricity =
    (this.energyData.electricity.grid || 0) +
    (this.energyData.electricity.renewable || 0);
  const fuel =
    (this.energyData.fuel.diesel || 0) +
    (this.energyData.fuel.petrol || 0) +
    (this.energyData.fuel.naturalGas || 0) +
    (this.energyData.fuel.lpg || 0);
...
emissionsSchema.virtual('totalWaste').get(function () {
  return (
    (this.wasteData.recyclable || 0) +
    (this.wasteData.hazardous || 0) +
    (this.wasteData.landfill || 0)
  );
});
